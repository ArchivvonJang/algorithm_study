...
public class 정렬_HIndex {
    public int solution(int[] citations) {
        ...
        int h;
        // 오름차순 정렬했으니까 앞에 있는 것보다 뒤에있는 수는 당연히 더 많은 횟수로 논문이 인용된 것이므로 논문 인용 횟수를 생각&비교할 필요가 없어진다.
        ...
        for (int i=0; i < citations.length; i++) {
            ...
            if (citations[i]>=h) {      // 논문 인용 횟수 citations[i] 가 h 이상인 논문 h편 이상
                answer = h;      // h의 최댓값
                break;
            }
        }
...
//        Stream.of(citations)
//                        .sorted().filter(numCitations -> numCitations == citationsSize)
