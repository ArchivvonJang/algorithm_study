...
public class 방의개수 {
	public static int solution(int[] arrows) {
        ...
        Map<Node, List<Node>> visited = new HashMap<방의개수.Node, List<Node>>();
        
        for(int arrow : arrows) {
        	for (int i = 0; i <= 1; i++) {//교차점 처리를 위한 스케일업 
        		Node nextNode = new Node(currentNode.x + dx[arrow], currentNode.y + dy[arrow]);
        		
        		//처음 방문하는 경우 = map에 키값이 없는 경우
        		if(!visited.containsKey(nextNode)) {
        			//리스트에 연결점 추가
        			visited.put(nextNode, makeEdgeList(currentNode));
        			
        			if(visited.get(currentNode) == null) {
        				visited.put(currentNode, makeEdgeList(nextNode));
        			} else {
        				visited.get(currentNode).add(nextNode);
        			}
        			
        		// 해당 정점을 재방문했고, 간선을 처음 통과하는 경우 	
        		} else  if(!visited.get(nextNode).contains(currentNode)){
